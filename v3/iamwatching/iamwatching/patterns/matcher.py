...
class PatternMatcher:
    ...
    async def _run_query(self, cypher: str) -> list[dict]:
    ...
    async def run_rule(self, rule: tuple) -> Finding:
        ...
        try:
            records = await self._run_query(cypher.strip())
        ...
        if use_registry:
            try:
                ...
                for check in yaml_checks:
                    ...
                    finding = await self.run_rule(rule_tuple)
                    if finding.affected_nodes:
                        findings.append(finding)
